...
class CustomFrozenLakeEnv(gym.Env):
    def __init__(self, size=8, num_holes=8, start_point=(0, 0), end_point=None):
    ...
    def generate_random_environment(self):
        ...
        np.random.shuffle(hole_positions)
        hole_positions = hole_positions[:self.num_holes]
    ...
    def step(self, action):
        ...
        reward = 1 if self.desc[row, col] == 'G' else 0  # 1 if the goal is reached, 0 otherwise
    ...
    def render(self):
        return [list(row) for row in self.desc]
...
# list_of_lists = env.render()

# # Concatenate strings within each sublist
# list_of_strings = [''.join(sublist) for sublist in list_of_lists]
...
# training script for map generated
...
# ppo_agent.save(os.path.join(model_save_path, "level2_custom_frozenlake_model"))
